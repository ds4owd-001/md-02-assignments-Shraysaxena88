...
5.  What are the data types of the three variables used for aesthetic mappings?
    ...
    The three variables used for aesthetic mapping are continuous (lifeExp), discrete (year) and categorical (continent).
...
# from stack overflow (https://stackoverflow.com/questions/5677885/ignore-outliers-in-ggplot2-boxplot)
...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here:
    ...
    Geom_bar by default counts the number of observations while geom_col plots the values in the data. Since the data plotted with geom_col is the count of observations from gapminder_2007, the plots are identical.

```{r}
ggplot(data=gapminder_2007,
      mapping=aes(x=continent))+
geom_bar()
#https://ggplot2.tidyverse.org/reference/geom_bar.html#arguments
```
...
6.  Describe how the `geom_histogram()` function is similar to the `geom_bar()` function.
    ...
    The geom_histogram function also counts the number of observations similar to geom_bar. However, histograms are more suitable for continuous variables, while bar plots are better for discrete variables.
...
7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe:
    ...
    The breaks argument changes the start, the end and the bin width of the histogram. Here for example, the histogram starts at life expectancy of 40, ends at 85 and is divided equally into bins with a bin width of 2.5 life expectancy.
...
4.  What is stored in the `country_colors` object? Find out by executing the object in the Console (type it to the Console and hit enter). Do the same again, but with a question mark `?country_colors`.
    ...
    Color schemes for 142 countries and 5 continents in the gapminder dataset.
